...
public class FileSystemContext implements Closeable {
  ...
  private synchronized void initContext(ClientContext ctx,
    ...
    LOG.error("---- DEBUG", new RuntimeException("---- Inited mFileSystemMasterClientPool: " + mFileSystemMasterClientPool));
  ...
  private synchronized void closeContext() throws IOException {
    if (!mClosed.get()) {
      ...
      LOG.error("--- Close", new RuntimeException("--- Closed mFileSystemMasterClientPool"));
